// =============================================
//  GUARDIANES DEL HUMEDAL — game.js v7
//  + Pisar enemigos, plataformas, recolectables,
...
function recalcPiso() {
  ...
  estado.PISO = canvas.height - estado.guardian.h - botonesH;
...
// Escala dinámica según pantalla (referencia: 900px de ancho)
// En un celular de 390px: ~0.5x | En tablet 768px: ~0.85x | En PC 1400px: ~1.4x
function sc(base) {
  return Math.round(base * Math.min(1.4, Math.max(0.45, canvas.width / 900)));
}

const NIVELES = [
  { nombre:'Amanecer en el Humedal', velBase:3.2, intervalo:130 },
  { nombre:'Tarde nublada',          velBase:4.0, intervalo:110 },
  { nombre:'Noche de luna',          velBase:5.0, intervalo:90  },
];
...
function loop(){
  ...
  if (!estado.pausado) {
    ...
    // ── Generar enemigos ──
    estado.timerE++;
    const intv=Math.max(45,nv.intervalo-Math.floor(estado.puntos/12)*2);
    ...
    if(g.inv===0&&!estado.pregActiva){
      for(let i=estado.enemigos.length-1;i>=0;i--){
        ...
        if(stomping(g,e)){
          ...
          mostrarMsg(`👟 ¡Aplastado! +15 pts — ${e.msg}`);
    ...
    // ── Nivel y dificultad ──
    estado.nivel=estado.puntos<70?0:estado.puntos<150?1:2;
